...
MULTI_STRATEGY_ROUTER_PROMPT = """Определи стратегию обработки запроса пользователя для системы Travel Agent.

ДОСТУПНЫЕ СТРАТЕГИИ:

1. "simple" - ПРОСТОЙ ВОПРОС (прямой ответ без инструментов):
   ...
   - Общая информация из базы знаний
...
2. "react" - ИССЛЕДОВАТЕЛЬСКИЙ ПОДХОД (гибкий, неопределенные задачи):
   - Открытые запросы без четкого плана
   - Исследование вариантов, подбор
   - "Что-нибудь", "возможно", "интересное"
   - Нужна гибкость в действиях
   - Задачи требующие адаптации на ходу

   Примеры REACT:
   - "Исследуй варианты отдыха в Юго-Восточной Азии"
   - "Подбери что-нибудь интересное в Европе"
   - "Найди варианты путешествия на выходные"
   - "Попробуй найти хорошие направления в Азии"
   - "Посоветуй куда поехать весной"

3. "plan_execute" - СТРУКТУРИРОВАННЫЙ ПОДХОД (четкий план):
   - Конкретные задачи с определенными шагами
   - Поиск рейсов, проверка погоды
   - Планирование с известными параметрами
   - Можно составить план заранее
   - Параллельное выполнение задач

   Примеры PLAN_EXECUTE:
   - "Найди рейсы из Москвы в Париж на 20 января"
   - "Какая погода в Берлине?"
   - "Спланируй: погода + рейсы + отель в Токио"
   - "Проверь погоду в Милане и Риме"
   - "Найди билеты Москва→Париж на следующую неделю"

КРИТЕРИИ ВЫБОРА:
- SIMPLE: Нет инструментов, ответ из знаний LLM
- REACT: Неопределенность, исследование, гибкость
- PLAN_EXECUTE: Четкая задача, можно спланировать шаги
- При сомнении между REACT и PLAN_EXECUTE → выбирай REACT
- Confidence < 0.7 → используй REACT (самая гибкая стратегия)
...
{{
    "type": "simple" | "react" | "plan_execute",
    "confidence": 0.0-1.0,
    "reasoning": "краткое объяснение решения (1-2 предложения)",
    "strategy_reasoning": "почему именно эта стратегия подходит",
...
class MultiStrategyRouter:
    """
    Мульти-стратегический маршрутизатор запросов для Travel Agent.

    Классифицирует запросы на 3 стратегии:
    - SIMPLE: прямой ответ без инструментов
    - REACT: гибкий исследовательский подход
    - PLAN_EXECUTE: структурированный подход с планом
    ...
    def __init__(
        ...
        # Создаём prompt template для мульти-стратегии
        self.prompt = ChatPromptTemplate.from_template(MULTI_STRATEGY_ROUTER_PROMPT)

        logger.info(
            f"MultiStrategyRouter initialized (heuristics={use_heuristics}, "
    ...
    def _heuristic_classification(self, query: str) -> Optional[RouteDecision]:
        ...
        # Проверяем PLAN_EXECUTE паттерны (самые специфичные, проверяем первыми)
        for pattern in self.PLAN_EXECUTE_PATTERNS:
            if pattern in query_lower:
                logger.info(f"Heuristic: PLAN_EXECUTE (matched pattern: '{pattern}')")
                return RouteDecision(
                    route_type=RouteType.PLAN_EXECUTE,
                    confidence=0.85,
                    reasoning=f"Запрос содержит паттерн '{pattern}' - структурированная задача",
                    strategy_reasoning="Четкая задача с определенными шагами",
                    direct_answer=None
                )

        # Проверяем REACT паттерны
        for pattern in self.REACT_PATTERNS:
            if pattern in query_lower:
                logger.info(f"Heuristic: REACT (matched pattern: '{pattern}')")
                return RouteDecision(
                    route_type=RouteType.REACT,
                    confidence=0.80,
                    reasoning=f"Запрос содержит паттерн '{pattern}' - исследовательская задача",
                    strategy_reasoning="Открытый запрос, нужна гибкость",
                    direct_answer=None
                )
        ...
        for pattern in self.SIMPLE_PATTERNS:
            if pattern in query_lower:
                ...
                return RouteDecision(
                    ...
                    strategy_reasoning="Ответ из базы знаний, инструменты не нужны",
    ...
    async def _llm_classification(self, query: str) -> RouteDecision:
        ...
        try:
            ...
            decision = RouteDecision(
                ...
                strategy_reasoning=result_dict.get("strategy_reasoning"),
            ...
            # Fallback - используем REACT (самая гибкая стратегия)
            return RouteDecision(
                route_type=RouteType.REACT,
                confidence=0.5,
                reasoning="Не удалось распарсить ответ LLM, используем REACT как fallback",
                strategy_reasoning="REACT как fallback при ошибке парсинга",
                direct_answer=None
            )
            ...
            return RouteDecision(
                route_type=RouteType.REACT,
                confidence=0.5,
                reasoning=f"Ошибка классификации: {str(e)}, используем REACT как fallback",
                strategy_reasoning="REACT как fallback при ошибке классификации",
                direct_answer=None
            )
    ...
    async def route(self, query: str) -> RouteDecision:
        ...
        if decision.confidence < self.confidence_threshold:
            logger.warning(
                ...
                "routing to REACT as fallback"
            )
            decision = RouteDecision(
                route_type=RouteType.REACT,
                confidence=decision.confidence,
                reasoning=f"Низкая уверенность ({decision.confidence:.2f}), используем REACT",
                strategy_reasoning="REACT выбран из-за низкой уверенности",
                direct_answer=None
            )
        ...
        if decision.route_type == RouteType.SIMPLE and decision.direct_answer is None:
            ...
            try:
                ...
                # Fallback - route to REACT
                decision = RouteDecision(
                    route_type=RouteType.REACT,
                    confidence=0.5,
                    reasoning=f"Не удалось получить прямой ответ: {str(e)}",
                    strategy_reasoning="REACT как fallback при ошибке получения ответа",
                    direct_answer=None
                )
...
QueryRouter = MultiStrategyRouter
